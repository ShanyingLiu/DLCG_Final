...
3. **Train (or load)** — `train_model(config, is_multitask, ...)` in `main.py`.

   **Which model class is constructed:**

   | `--mode`   | `--multitask-arch` | Class instantiated                  | Source                          | Checkpoint tag    |
   | ---------- | ------------------ | ----------------------------------- | ------------------------------- | ----------------- |
   | baseline   | (ignored)          | `BaselineLightingNet(config)`       | `code/models/baseline_model.py` | `baseline`        |
   | multitask  | `tiled` (default)  | `MaterialAwareLightingNet(config)`  | `code/models/multitask_model.py`| `multitask`       |
   | multitask  | `film`             | `MaterialGuidedLightingNet(config)` | `code/models/multitask_model.py`| `multitask_film`  |

   With `--mode both` the baseline runs first then the multitask variant
   selected by `--multitask-arch`. All three classes share the same backbone
   recipe described in **Model architectures** below: ImageNet-pretrained
   ResNet-50, everything before `layer4` frozen (`requires_grad=False` for any
   param whose name doesn't start with `'7'`, ResNet's layer4 child index).

   **Loss criterion:**
   - `is_multitask=True`  → `MultiTaskLoss(config)` from
     `code/training/losses.py` (log-HDR weighted-MSE + weighted-L1 +
     (1−SSIM) on the envmap, plus MSE on the material 5-vector). Per-term
     weights from `config.lighting_{mse,l1,ssim}_weight` and
     `config.material_loss_weight`.
   - `is_multitask=False` → `criterion=None`; `Trainer` calls the standalone
     `lighting_loss(...)` directly with the same per-term weights, no
     material term.

   **Optimizer + schedule (identical for all three classes):**
   - `AdamW(filter(requires_grad, model.parameters()),
            lr=config.learning_rate, weight_decay=config.weight_decay)`
     — frozen backbone params are excluded by the `filter` so AdamW doesn't
     allocate state for them.
   - `CosineAnnealingLR(optimizer, T_max=config.num_epochs)` — single cosine
     decay from `lr` to ~0 over the full training run, no warmup, no restarts.

   **Trainer loop** (`code/training/train.py`):
   - `Trainer.fit` iterates `1..config.num_epochs`. Each epoch:
     `train_epoch` (forward → loss → backward → `optimizer.step()`) →
     `validate` (no grad; also tracks `material_mae` on the multitask path) →
     `scheduler.step()` → checkpoint if `val_loss` improves.
   - Checkpoint path:
     `{config.save_dir}/{config.experiment_name}_{tag}_best.pt`
     where `tag` is the value from the table above (chosen inside
     `Trainer._save_checkpoint` from `is_multitask` and
     `config.multitask_arch`). Saved dict has
     `{epoch, model_state_dict, optimizer_state_dict, val_loss}`.
   - A `{exp}_{tag}_curves.png` figure is written from the returned history.

   **`--eval-only` path:** skips `train_model` entirely. `_load_checkpoint(...)`
   constructs a fresh instance of the same class via the same selection logic,
   calls `load_state_dict` from `<exp>_<tag>_best.pt`, moves to device, sets
   `eval()` mode. No optimizer, no scheduler, no training history.
